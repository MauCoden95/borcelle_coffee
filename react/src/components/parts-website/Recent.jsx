    ...
    <div>
      <div className='w-full bg-orange-100 p-1'>
        <div className='w-5/6 min-h-0 m-auto'>
          ...
          <div className='w-5/6 min-h-0 m-auto my-10 flex items-center justify-between'>
            {products.map((p) => (
              <div className='relative w-1/5 h-80 bg-transparent rounded-xl border-2 border-gray-400 p-2'>
                <img className='absolute top-0 left-1/2 transform -translate-x-1/2 block w-28' src={`http://localhost:8000/img/${p.img}`} alt={p.img} />
                <p className='absolute bottom-10 left-1/2 transform -translate-x-1/2 text-center text-sm sm:text-base my-7'><h2 className='block text-base sm:text-2xl'></h2> {p.name}</p>
